...
GitHub Releases is the only provider currently supported. S3 and Generic
HTTP are planned for a future release and are not yet wired into the
build.
...
> **Important**
>
> S3 is planned but not yet supported.
> [`enable_auto_updates()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/enable_auto_updates.md)
> currently accepts only `"github"` as the provider, and S3
> configuration in `_shinyelectron.yml` is not consumed by the build.
> This section documents the intended design for when S3 support is
> added.
...
> **Important**
>
> Generic HTTP is planned but not yet supported.
> [`enable_auto_updates()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/enable_auto_updates.md)
> currently accepts only `"github"` as the provider, and generic HTTP
> configuration in `_shinyelectron.yml` is not consumed by the build.
> This section documents the intended design for when Generic HTTP
> support is added.
