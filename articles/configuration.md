...
The shinyelectron package supports a YAML configuration file that
simplifies the export process by storing settings in your project
directory. This guide explains the configuration file format and all
available options.

## Why Use a Configuration File?

Instead of passing many parameters to
...
You can store these settings in a `_shinyelectron.yml` file and simply
run:
...
The configuration file must be named `_shinyelectron.yml` and placed in
the root of your Shiny application directory:
...
>
> Building for a platform different from your development machine may
> have limitations. macOS apps can only be built on macOS. Windows and
> Linux apps can be cross-compiled with additional setup.
...
> (macOS), [ImageMagick](https://imagemagick.org/), or online converters
> can create platform-specific icon formats from a single
> high-resolution PNG.
...
> **Auto-Install Opt-In**
>
> Auto-install is disabled by default. Set `auto_install: true`
> explicitly to enable automatic Node.js installation during
> [`export()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/export.md).

### `r` Section

Pins the R version used for `bundled` and `auto-download` runtime
strategies.
...
Pins the Python version used for `bundled` and `auto-download` runtime
strategies.
...
Configures the container backend when `runtime_strategy` is
`"container"`.
...
Electron shell with a launcher screen. Requires at least two entries.
Each app can override the default `build.type`.
...
When you call
[`export()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/export.md)
with both a config file and function parameters, values are merged with
this precedence (highest to lowest): 1. **Function parameters** - Values
passed directly to
[`export()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/export.md)
2. **Configuration file** - Values from `_shinyelectron.yml` 3.
...
If the YAML file has syntax errors, shinyelectron warns and uses all
defaults.
