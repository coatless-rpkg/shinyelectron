...
The shinyelectron package can manage its own Node.js installation,
separate from any system-wide installation. This guide explains how to
install, manage, and troubleshoot Node.js within shinyelectron.

## Why Local Node.js?

Managing Node.js locally through shinyelectron provides several
advantages:
...
shinyelectron reports on local Node.js installations in system
diagnostics:
...
The most recently installed version (or the latest version if versions
are equal) is used by default.
...
This ensures the downloaded files haven’t been corrupted or tampered
with.
...
will automatically install Node.js if not found.
...
If the problem persists, nodejs.org may be having issues. Wait and try
again later.

### Wrong Architecture

**Symptom:** Node.js runs but Electron fails with architecture mismatch
errors.
...
|-----------------|-------------------------------------------------------------------------------------------------------------------|------------------------------------|
| Installation    | [`install_nodejs()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/install_nodejs.md)            | Download from nodejs.org           |
| Location        | `~/.shinyelectron/cache/`                                                                                         | `/usr/local/`, `C:\Program Files\` |
| Permissions     | User-only                                                                                                         | Often requires admin               |
| Updates         | Manual via [`install_nodejs()`](https://r-pkg.thecoatlessprofessor.com/shinyelectron/reference/install_nodejs.md) | System package manager             |
| Isolation       | Per-user, isolated                                                                                                | System-wide, shared                |
| Version control | Multiple versions supported                                                                                       | Usually one version                |

For most shinyelectron users, the local installation is recommended
because it’s isolated and reproducible.
...
- **[Configuration](https://r-pkg.thecoatlessprofessor.com/shinyelectron/articles/configuration.md)**:
  Set Node.js options in config files
