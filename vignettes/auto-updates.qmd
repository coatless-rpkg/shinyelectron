...
Automatic updates allow your Electron application to check for, download, and
install new versions seamlessly. This guide explains how to configure
auto-updates using shinyelectron and electron-updater.

## Overview

shinyelectron uses [electron-updater](https://www.electron.build/auto-update)
to provide automatic updates. electron-updater is the library electron-builder
uses to check for, download, and install updates. Updates can be distributed
via:
...
Use `enable_auto_updates()` to configure your app:
...
::: {.callout-note}
The file names must match the pattern `{productName}-{version}.{ext}` for
electron-updater to detect them correctly.
:::

### Step 4: Test Updates

When you publish a new release with a higher version number, the app will:
...
These three settings control the update pipeline: `check_on_startup` asks "is
there an update?", `auto_download` fetches it, and `auto_install` applies it.
...
For private GitHub repositories, users need a `GH_TOKEN` environment variable:
...
**Solutions:**
1. Ensure app has write permissions to its directory
2. On macOS, app must be in Applications folder
3. Check if antivirus is blocking the update
...
**Solutions:**
1. Verify version in `package.json` matches release tag
2. Ensure release artifacts follow naming convention
3. Check that release is published (not draft)
...
- **[GitHub Actions](github-actions.html)**: Automated CI/CD builds
